...
> **보존 상태:** 이 문서는 당시 사용한 LED 제어 인터페이스를 설명합니다. 문서에 기록된 커스텀 OpenCR 펌웨어의 최종 소스와 바이너리는 원본 폴더에서 발견되지 않았으며, `tools/opencr_updater/`의 stock 바이너리는 이를 대체하지 않습니다.
...
- 각 LED에 직렬 저항을 사용한다.
- 배선 변경 전에는 Jetson과 OpenCR 전원을 끈다.
- LED의 `+`를 GPIO에, `-`를 GND에 연결한다.
...
점멸 주기는 약 0.5초이다. 범위를 벗어난 값은 Orin 노드에서 무시된다.

## 4. TurtleBot3 노드 실행

새 터미널에서는 ROS 2와 로컬 TurtleBot3 workspace가 `~/.bashrc`를 통해
자동으로 적용된다. 수동으로 적용해야 할 때는 다음 명령을 사용한다.
...
일반적인 TurtleBot3 bringup을 실행한다.

```bash
ros2 launch turtlebot3_bringup robot.launch.py
```

LiDAR 없이 TurtleBot3 노드만 시험할 때는 다음과 같이 실행할 수 있다.
...
만들어 `UInt8` 메시지를 직접 발행하는 방식을 권장한다.
...
publisher는 노드가 시작될 때 한 번 생성하고, 이벤트마다 `publish()`만 호출한다.
...
패키지 경로는 다음과 같이 로컬 workspace를 가리켜야 한다.
...
1. OpenCR USB 연결과 전원을 확인한다.
2. OpenCR의 `RESET` 버튼을 짧게 한 번 누른다.
3. 다시 확인한다.
...
- 수정된 `turtlebot3_node`가 실행 중인지 확인한다.
- 터미널의 `ROS_DOMAIN_ID`가 다른 노드와 같은지 확인한다. 현재 기본값은 `30`이다.
- OpenCR GPIO 물리 핀 3, 4와 GND 배선을 확인한다.
- 노드 로그에 아래와 같은 메시지가 출력되는지 확인한다.
...
- OpenCR 펌웨어: `burger_led_control V260818R3`로 기록되어 있으나 소스·바이너리는 현재 보유하지 않음
