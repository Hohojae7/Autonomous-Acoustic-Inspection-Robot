...
- **위험 노출 저감**: 작업자가 설비 가까이에서 반복적으로 수행하던 1차 점검을 이동 로봇이 대신 수행
- **점검 절차 표준화**: 지정된 경로를 순찰하고 같은 처리 과정으로 설비음의 정상·이상 상태를 판정
- **현장 확인성 확보**: 판정 상태를 ROS 2 Terminal과 Green/Red LED로 즉시 출력
- **운용 유연성 확보**: 자율순찰이 어려운 구간에서는 제스처 또는 IMU 장갑으로 개입한 뒤 자율주행으로 복귀
- **통합 시스템 구현**: 센서 입력, 이동 제어, 음향 판정, 상태 출력을 ROS 2 중심의 하나의 운용 흐름으로 연결
